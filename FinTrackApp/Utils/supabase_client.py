...
class SupabaseStorage:
    ...
    def crear_bucket_ingresos(self):
        """Crea el bucket 'ingresos_img' si no existe"""
        try:
            self.client.storage.create_bucket(
                id='ingresos_img',
                name='ingresos_img',
    ...
    def upload_ingreso_image(self, file_bytes: bytes, file_name: str):
        ...
        try:
            ...
            response = self.client.storage.from_('ingresos_img').upload(
                path=path,
                file=file_bytes,
                file_options={"content-type": "image/jpeg"}
            )
            ...
            # Obtener URL pública
            url = self.client.storage.from_('ingresos_img').get_public_url(path)
        ...
        try:
            ...
            if not match:
                return {
                    'success': False,
                    'error': 'URL no corresponde a bucket de productos',
                    'details': f'URL no contiene /public/ingresos_img/: {image_url}'
                }
            ...
            # Eliminar del bucket 'productos'
            try:
                response = self.client.storage.from_('ingresos_img').remove([path])
